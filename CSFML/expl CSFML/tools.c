...
sfBool Rectangle_Collision(sfFloatRect _box1, sfFloatRect _box2) {
    if ((_box2.left >= _box1.left + _box1.width) || (_box2.left + _box2.width <= _box1.left) ||
        (_box2.top >= _box1.top + _box1.height) || (_box2.top + _box2.height <= _box1.top))
        return sfFalse;
    else
        return sfTrue;
}
...
void screenshot(sfRenderWindow* _window) {
    ...
    _strtime(hours);
    _strdate(date);
    sfVector2u windowSize = sfRenderWindow_getSize(_window);
